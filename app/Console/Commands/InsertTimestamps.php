...
class InsertTimestamps extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'insert:timetamps';
     ...
     * @var string
     */
    protected $description = 'Insert most common timpstamps to time_stamps table in DB';
    ...
    {
        if ($this->timestampService->count() > 0) {
            return $this->line('Table time_stamps in not empty!');
        }

        $timestamps = [];
        foreach ($this->timestamps() as $name => $timezone) {
            $timestamps[] = ['name' => $name, 'timezone' => $timezone];
        }

        $added = $this->timestampService->insertMany($timestamps);
        if ($added) {
            return $this->info('Timestamps inserted successfully');
        }

        return $this->error('Failed to insert timestamps');
    }
